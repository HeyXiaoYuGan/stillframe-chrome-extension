...
- 用户主动使用抖音功能时，读取当前 `douyin.com` 页面已经加载的作品标题、作者、封面、视频规格和临时媒体地址；不会读取抖音 Cookie 或把媒体地址发送给第三方解析服务。
- `onlyfans.com` 页面加载时，扩展会在页面主世界中检查页面自身收到的 OnlyFans JSON 响应副本，并只提取其中的视频 ID、标题、封面、规格和临时媒体地址。提取结果保留在页面内存中，并按当前标签页暂存在 `chrome.storage.session`，以便页面下滑加载新内容及工具栏弹窗重新打开后继续显示；不读取 Cookie、请求头、许可证或密钥，不保存完整响应，也不把媒体地址发送给第三方解析服务。Pornhub 数据仍只在用户主动识别时读取。
...
`chrome.storage.session` 临时保存当前后台任务状态、每个标签页最多 400 条通用媒体记录，以及最多 200 条 OnlyFans 视频列表记录。媒体记录可能包含网站生成的临时或签名 URL；它们只用于扫描、列表恢复、预览和用户发起的下载，在标签页导航、标签页关闭或浏览器会话结束时清除，不写入长期存储。OnlyFans 页面监听器还会在当前网页的 JavaScript 内存中保留最近 100 条提取结果，刷新或关闭页面即清除。
...
- 当前 `douyin.com` 页面及页面已经提供的抖音、字节媒体 CDN；
- 当前 `onlyfans.com`、Pornhub 官方页面及页面已经提供的媒体 CDN；
...
StillFrame 不伪造会员、购买或账户权限，不破解 DRM、签名或加密媒体，也不绕过登录、付费、会员、地区或网站访问限制。B站规格仅来自当前浏览器会话实际获得的接口结果；Pinterest 请求使用当前页面已有权限；OnlyFans 与 Pornhub 功能仅处理当前页面已加载且当前账户能够正常播放的媒体。

## 用户控制

用户可以在设置页关闭实时扫描或系统通知。关闭实时扫描后，扩展不会在页面初始化时自动扫描，也不会自动收集后续 DOM 变化、媒体加载事件、性能条目或网络媒体请求；用户主动点击扫描、打开媒体画廊、截图，或使用 Pinterest、抖音、OnlyFans、Pornhub、B站和下载功能时仍可按需读取当前页面及相关媒体。后台会继续在主框架导航时清除上一页面的临时媒体记录。重新开启实时扫描会在无需刷新页面的情况下扫描一次当前页面，并继续监听新媒体。
