...
本文件依据当前仓库中的 `manifest.json` 权限清单维护。权限仅用于扩展现有的图片提取、通用媒体扫描、截图、ZIP、Pinterest 和 B站功能；发布前应以同目录的 Manifest 为准进行核对。

## 扩展权限

| 权限 | 用途 |
| --- | --- |
| `contextMenus` | 创建图片、视频、打包、截图、Pinterest 和 B站操作的右键菜单。 |
| `declarativeNetRequestWithHostAccess` | 仅创建一条 B站媒体下载规则：访问 `bilivideo.com` / `bilivideo.cn` 媒体时设置 `Referer: https://www.bilibili.com/`，使 Chrome 下载与页面正常播放请求保持相同来源边界。它不修改 Cookie、Authorization 或响应内容。 |
...
| `scripting` | 为完整页面截图临时执行页面滚动/尺寸读取，并读取当前 B站页面已公开给播放器的标题、BV/AV/CID 等上下文。 |
| `storage` | 在 `local` 保存设置，在 `session` 保存短期任务状态和按标签页划分的临时媒体 URL。详细规则见 [PRIVACY.md](PRIVACY.md)。 |
| `webRequest` | 仅在实时扫描开启时观察网页媒体请求 URL、资源类型和响应 `Content-Type`，用于发现 MP4、WebM、HLS、Pinterest 媒体和 B站 DASH。关闭实时扫描后不新增网络媒体记录，仅保留页面导航时的临时记录清理。不会读取请求正文、Cookie 或 Authorization 请求头。 |
...
- 支持 Pinterest、B站以及普通网站使用不同媒体域名和临时 CDN 的情况。
