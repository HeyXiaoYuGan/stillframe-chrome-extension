...
本文件依据当前仓库中的 `manifest.json` 权限清单维护。权限仅用于扩展现有的图片提取、通用媒体扫描、截图、ZIP、Pinterest、抖音、OnlyFans、Pornhub 和 B站功能；发布前应以同目录的 Manifest 为准进行核对。
...
- **OnlyFans / Pornhub**：OnlyFans 专用主世界脚本在页面加载时检查页面自身收到的 JSON 响应副本，只提取视频元数据和临时媒体地址，并按当前标签页短期保存在页面内存与 `chrome.storage.session`，用于页面下滑加载和工具栏弹窗重新打开后的列表恢复；用户主动打开“神秘小入口”时还会结合视频元素与媒体请求重新识别。Pornhub 仍仅在用户主动操作时解析。不读取 Cookie、请求头、许可证或密钥，不使用第三方解析网站，不解密加密媒体。
